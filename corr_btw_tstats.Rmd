...
run_simulation = function(nmax, ngenes, cor.target, nvalues, hsq, nreps=100, BPPARAM=SnowParam()){
  ...
  df = lapply(apply(grd, 1, as.list), function(x, geneExpr1, geneExpr2, info){
...
ngenes = 1000 
cor.target = .8
nvalues = c(5, seq(10, 40, by=10), seq(50, 100, by=30), seq(150, 300, by=50))
hsq = 0.6
  
df_param = expand.grid(hsq = c(0.05, 0.10), 
            cor.target = c(  0, 0.3, 0.6, .95))
# -0.95, -0.6, -0.3,
...
run_simulation = function(nmax, ngenes, cor.target, nvalues, hsq, nreps=100, BPPARAM=SerialParam()){
