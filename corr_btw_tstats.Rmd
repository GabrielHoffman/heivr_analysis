...
run_simulation = function(nmax, ngenes, cor.target, nvalues, hsq, nreps=100, BPPARAM=SnowParam()){
  ...
  # Simulate errors
  geneExpr1 = matrix(NA, nrow(Eta1), ncol(Eta2))
  geneExpr2 = matrix(NA, nrow(Eta2), ncol(Eta2))

  for(j in seq(nrow(Eta1)) ){
    se1 = sqrt((1-hsq) / hsq * var(geneExpr1[j,]))
    geneExpr1[j,] = rnorm(nmax, 0, se1)

    se2 = sqrt((1-hsq) / hsq * var(geneExpr12[j,]))
    geneExpr2[j,] = rnorm(nmax, 0, se2)
  }
