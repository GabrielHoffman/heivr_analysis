...
frac = c(seq(.05, .1, length.out=8), seq(.15, 1, length.out=10))
...
df.ramp = lapply(coefTest, function(coef){
  message(coef)
  get_rho_rampup(coef, frac, nreps=10, BPPARAM = SnowParam(6))
  })
...
res2 %>%
  ggplot(aes(fraction + offset, rho.mean, color=Method)) +
    ...
    geom_point() + 
    geom_line() + 
    geom_errorbar(aes(ymin = pmax(-1,rho.sem - 1.96*rho.sem),
                       ymax = pmin(1, rho.sem + 1.96*rho.sem)),
                  width=0) +
    theme_bw() +
