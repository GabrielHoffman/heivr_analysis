...
for(coef in names(hobjList)){
  hobj = hobjList[[coef]]
  lim = range(c(hobj$x, hobj$y))

  plot(hobj, xlab="logFC (MSSM-Penn-Pitt)", ylab="logFC (NIMH-HBCC)", main=coef, asp = 1)
  abline(h=0, lty=3)
  abline(v=0, lty=3)
  abline(0,1)
} 
...
df.ramp = lapply(coefTest, function(coef){
  message(coef)
  get_rho_rampup(coef, frac, nreps=20, BPPARAM = SnowParam(12))
  })
