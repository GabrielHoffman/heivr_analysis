...
/Users/gabrielhoffman/workspace/repos/heivr_analysis
...
```{r run_ramp, message=TRUE}
# ramp up analysis
coefTest = c('DxSCZ', 'Reported_GenderMale', 'ageOfDeath')

df.ramp = lapply(coefTest, function(coef){
  get_rho_rampup(coef, seq(.1, 1, length.out=8), nreps=20, BPPARAM = SnowParam(4, progressbar=TRUE))
  })
df.ramp = do.call(rbind, df.ramp)
```

```{r rampup.plot.SCZ, cache=FALSE}
res2 = df.ramp %>%
...
res2$offset = sapply(as.character(res2$Method), function(x) 
                switch(x, heivr = -0.01, Pearson=0, MOM=0.01))
...
ggplot(res2, aes(fraction + offset, rho.mean, color=Method)) +
    ...
    xlab("Fraction of subjects used from each study") +
