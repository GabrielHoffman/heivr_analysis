  ...
  res = bplapply(1:nreps, function(k, vobj.lst, METADATA){
    ...
    res = lapply( fractions, function(frac){
      ...
      fit_marginal.lst = lapply( vobj.lst, function(vobj){
        ...
        form_mod = dreamlet:::removeConstantTerms(form_use, info[idx,])

        # Drop variables in a redundant pair
        form_mod = dreamlet:::dropRedundantTerms(form_mod, info[idx,])

        fit = dream(vobj[,idx], form_mod, info[idx,], quiet=TRUE)  
        eBayes( fit )
        })
