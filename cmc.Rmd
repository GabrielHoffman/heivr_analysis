...
get_rho_rampup = function( coef, fractions=seq(.2, 1, length.out=3), nreps=10, BPPARAM = SnowParam(4, progressbar=TRUE) ){
  ...
  res = lapply(1:nreps, function(k, vobj.lst, METADATA, fractions){
    ...
    res = lapply( fractions, function(frac){
      ...
      # MLE
      # saveRDS(df, file = "/sc/arion/scratch/hoffmg01/df")
...
coefTest = c( 'Reported_GenderMale', 'ageOfDeath')
frac = c(seq(.03, .13, length.out=12), seq(.15, 1, length.out=20))
...
df.ramp = lapply(coefTest, function(coef){
  message(coef)
  get_rho_rampup(coef, frac, nreps=100, BPPARAM = SnowParam(1))
  })
...
res2 %>%
  ggplot(aes(fraction + offset, rho.mean, color=Method)) +
    ...
    geom_point() + 
    geom_line() + 
    geom_errorbar(aes(ymin = pmax(-1,rho.mean - 1.96*rho.sem),
                       ymax = pmin(1, rho.mean + 1.96*rho.sem)),
                  width=0) +
    theme_bw() +
