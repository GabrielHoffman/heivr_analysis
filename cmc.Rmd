...
get_rho_rampup = function( coef, fractions=seq(.2, 1, length.out=3), nreps=10, BPPARAM = SnowParam(4, progressbar=TRUE) ){
  ...
  res = bplapply(1:nreps, function(k, vobj.lst, METADATA, factions){
    ...
    res = lapply( fractions, function(frac){
      ...
      fit_marginal.lst = lapply( vobj.lst, function(vobj){
        ...
        # TODO: select case/control and male/female
        idx = sample.int( ncol(vobj), ncol(vobj)*frac)

        info.sub = droplevels(info[idx,])

        form = update(form.lst[[info$Cohort[1]]], ~ . - Dx - Reported_Gender - ageOfDeath)
        dsgn = model.matrix(form, info.sub)
        dcmp = svd(dsgn)
        # incl = seq( max(which(dcmp$d > 1e-6)) - 2)
        incl = which(cumsum(dcmp$d^2/ sum(dcmp$d^2)) < .999)
        data2 = with(dcmp, u[,incl,drop=FALSE] %*% diag(d[incl]))
        data2 = data.frame(data2)
        data2 = data.frame(with(info.sub, data.frame(Dx,Reported_Gender, ageOfDeath)), data2)
        rownames(data2) = colnames(vobj[,idx])
        form_mod = as.formula(paste0('~ Dx + Reported_Gender + ageOfDeath + ', paste0(colnames(data2)[-c(1:2)], collapse=" + ")) )

        tryCatch({
          fit = dream(vobj[,idx], form_mod, data2, quiet=TRUE)  
          fit = eBayes( fit )
...
# frac = c(seq(.02, .1, length.out=9), seq(.15, 1, length.out=3))

coefTest = c('DxSCZ')
frac = c(.1, .5)

df.ramp = lapply(coefTest, function(coef){
  get_rho_rampup(coef, frac, nreps=2, BPPARAM = SnowParam(4))
  })
