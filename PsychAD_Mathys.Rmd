...
tab.mathys = topTable(fit.mathys, coef="DxAD", number=Inf) %>%
                as_tibble %>%
                mutate(se = logFC / t)
```
...
lim = c(-lim, lim)
plot(hobj.mle, xlab="logFC (PsychAD)", ylab="logFC (Mathys)", main="Microglia")#, xlim=lim, ylim=lim)
abline(0,1)
abline(h=0, lty=3)
abline(v=0, lty=3)  
...
pheatmap(C[-i, -j], breaks=bks, cellheight=15, cellwidth = 15, color=col, breaklist=seq(-1, 1, by=0.5), main = "heivr")
```
