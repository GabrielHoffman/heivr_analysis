...
par(asp=1)
plot(hobj.mle, xlab="logFC (PsychAD)", ylab="logFC (Mathys)", main="Astrocytes")
...
par(asp=1)
plot(hobj.mle2, xlab="logFC (PsychAD)", ylab="logFC (Mathys)", main="Astrocytes: permuted")
```
...
heivrMatrix = function( tab ){
  ...
  df = lapply( seq(nrow(grd)), function(i){
    ...
    hobj.mle = with(tab.join, heivr(logFC.x, logFC.y, se.x^2, se.y^2, LRT=FALSE))
  ...
  C.pearson = sparseMatrix(i = as.numeric(df$i), 
                          ...
                          x = df$cor, 
                          symmetric = TRUE)
  rownames(C.pearson) = cellTypes
  colnames(C.pearson) = cellTypes
  diag(C.pearson) = 1
  ...
  C.heivr = sparseMatrix(i = as.numeric(df$i), 
                        ...
                        x = df$cor.heivr, 
                        symmetric = TRUE )
  rownames(C.heivr) = cellTypes
  colnames(C.heivr) = cellTypes
  diag(C.heivr) = 1

  list(C.pearson = as.matrix(C.pearson),
      C.heivr = as.matrix(C.heivr) )
}
...
# Mathys
```{r plots.Mathys}
ggcorrplot(res.Mathys$C.pearson, method = "circle",hc.order = TRUE)
ggcorrplot(res.Mathys$C.heivr, method = "circle", hc.order = TRUE)
```
...
# Merge
```{r plots.merge, fig.height=12, fig.width=12}
